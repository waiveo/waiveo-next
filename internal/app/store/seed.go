...
// schedule on the screen, and two non-overlapping dayparts partitioning the day
// (a content daypart 06:00–22:00 playing the playlist and firing a rising-edge
// preset, and a blank overnight daypart 22:00–06:00) — the same shape the former
// hardcoded feeder demo carried, so a fresh make dev still resolves a program,
// but now editable over the api.
//
...
// the daypart that references them, the schedule before its dayparts. Every
// accepted Create bumps the store generation in its own transaction, so a freshly
// seeded store lands at generation 7. assetRef is the content id the demo
// playlist item points at (the same asset the feeder's screen_programs item
// shows). SeedDemo is meant for an empty store — the feeder seeds only when the
