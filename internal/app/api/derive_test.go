...
// namedInlineSlidePlaylist wraps one layer stack in a `source: "slide"` playlist
// item under a caller-chosen name. It is the named sibling of
// playlistinlineslide_test.go's inlineSlidePlaylist, which the cases there use
// where the name does not matter.
func namedInlineSlidePlaylist(scopeNode, name string, layers []wire.Layer) datamodel.Playlist {
...
// plantPlaylistRow writes a playlist body straight into the database file
// through a second connection, bypassing every validator the store interposes.
//
// It is the api-package twin of store's plantMalformedPlaylist, and it exists
// for the same reason: the row under test is one an OLDER build, a restore or a
// seed put there, and the inline authoring gate
// (datamodel.checkPlaylistItems, added on the interactive-layers track) refuses
// to mint it through POST /playlists. A fixture that depended on the current
// build accepting it would stop testing anything the day that gate arrived —
// which is exactly what happened, and is why this plants instead.
func plantPlaylistRow(t *testing.T, dsn, id, scopeNode, name, items string) {
	t.Helper()
	body := `{"id":"` + id + `","scope_node":"` + scopeNode + `","name":"` + name + `",` +
	...
	if _, err := db.Exec(
		`INSERT INTO playlists (id, revision, external_id, labels, scope_node, created_at, updated_at, body)
		 VALUES (?, 1, '', '{}', ?, 1, 1, ?)`, id, scopeNode, body); err != nil {
...
// with the inline authoring gate in place the API cannot construct the input at
// all, so deleting the guard left the whole package green.
//
// So the row is PLANTED rather than POSTed. That is not a way around the gate,
// it is the honest fixture: the gate is upstream of the store and the queue is
// downstream of it, and a spec-less layer arrives from a restore, a seed or an
// older build — the shapes internal/app/store's malformedrowveto_test.go pins as
// genuinely storable.
//
...
func TestTheQueueOmitsASpecLessLayerRatherThanServingItNull(t *testing.T) {
	...
	const listID = "01J9DQUEUE0M1TSSPECLESSAAA"
	plantPlaylistRow(t, dsn, listID, screenID, "Half Broken", `[{"source":"slide","slide":{"layers":[`+
		...
		`{"kind":"derive","x":0,"y":500,"w":900,"h":300,`+
...
func TestAWellFormedInlineDeriveLayerIsStillAccepted(t *testing.T) {
	...
	listID := decodeID(t, e.createOK(t, "/api/v1/playlists", rowCreateBody(t,
		namedInlineSlidePlaylist(screenID, "Pending Inline", []wire.Layer{
...
func TestThePendingQueueConformsToItsDeclaredSchema(t *testing.T) {
	...
	e.createOK(t, "/api/v1/playlists", rowCreateBody(t,
		namedInlineSlidePlaylist(screenID, "Queue Shape Playlist", []wire.Layer{
