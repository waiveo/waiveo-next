...
**[PLY-024]** A pairing code (Pairing redemption) MUST deterministically decode, by the player accepting it, to a relay dial address `{host, port}` — a player MUST be able to reach a relay from a pairing code alone, with no dependency on same-network discovery having succeeded. A pairing code MUST be a fixed-length string over an alphanumeric alphabet, presented in hyphen-separated groups of equal length so an operator can key it on a bounded-input remote control, and sized to carry all three decoded components — the dial address, the `grant_selector`, and the `fingerprint_commitment` (PLY-053) — with the hyphens serving as visual grouping a decoder strips before decoding, never as payload.

*draft-note: the pairing code's exact character count (and the group size the hyphens impose) is not yet fixed, because it follows from a concrete encoding of the three decoded components not yet chosen: the decoded payload is the dial address plus the `grant_selector` plus the `fingerprint_commitment`, whose own length is fixed at no less than 80 bits (PLY-052). Once those component encodings are chosen the count follows deterministically, and MUST keep the whole code short enough for remote-control entry.*
...
**[PLY-052]** Under commitment-verified trust, a player MUST compute a SHA-256 digest over the canonical concatenation, in `trust_anchors` array order, of each entry's trust-anchor public key (the SubjectPublicKeyInfo its `pem` certifies — not the surrounding certificate's other fields), truncate that digest to its leading `fingerprint_commitment`-length bits, and compare the result byte-exact against the `fingerprint_commitment` it holds (PLY-051). A match is this attempt's positive trust decision; any mismatch is an authentication failure (PLY-057). The `fingerprint_commitment` length MUST be at least 80 bits: at that truncation the second-preimage search an attacker would need — to construct trust-anchor material whose SubjectPublicKeyInfo hashes to a given commitment — is beyond practical reach, while the value still fits in a short additional block of a human-typed pairing code (PLY-024).
...
**[PLY-066]** A player MUST persist a clock floor — the latest time value it has verified or observed as current — and on restart MUST NOT adopt a wall-clock reading earlier than that floor, mirroring the floor a relay persists for its own untrusted-clock evaluation (`relay/1` REL-130). The floor MUST advance only from a time the player can verify against something other than its own unattested local reading — for instance, time established over a successfully certificate-validated session (PLY-060) — mirroring the independently-verifiable-source rule `relay/1` REL-132 states for a relay. This floor governs only the player's own credential-temporal evaluation: the certificate-validity window a pinned connection checks (PLY-060–061), and the player's own assessment of how near its channel token is to `expires_at` for renewal timing (PLY-074). It bounds a first-boot clock skew (PLY-048) and prevents a clock rewound below known-good from reviving an expired credential or deferring a due renewal. It MUST NOT be applied to Lease-validity evaluation, which PLY-098 deliberately performs against the raw local clock so a screen keeps playing across a clock fault.
...
**[PLY-124]** A screen — the identity row a player's `screen_id` names (`data-model/1` DAT-004), placed in the scope-node tree — MAY carry a reference to the `device_id` of an adopted device (`relay/1` REL-063) representing the same physical display. This optional reference is the linkage a status report's upstream fan-out keys on: where it is present, a screen's `POST /player/v1/status` report (PLY-120) is the upstream source of that `device_id`'s `device.heartbeat` telemetry (`events/1` EVT-060), exactly as `render/end` sources `content.played` (PLY-113); where it is absent, a status report still serves this contract's own liveness and recovery purposes (Screen liveness and recovery) and produces no `device.heartbeat` event. A screen and an adopted device remain distinct rows, and neither this reference nor its absence changes a player's own status-reporting obligations (PLY-120–123).
...
- The pairing code's exact character count (PLY-024's remaining draft-note), the ordinary and long-poll pull cadences (PLY-082), the status-report interval (PLY-120), the reconnect backoff curve and re-locate multiple (PLY-131–132), and the recovery settle delay (PLY-154) are all draft-note proposals pending real measurement; corpus cases exercise the shapes and orderings these rules produce, not elapsed real time. The fingerprint-commitment length (PLY-052, at least 80 bits) and the channel-token expiry bound (PLY-071, at most 24 hours) are now fixed normatively rather than left to measurement.
- Whether a given screen's status reports are also forwarded as `events/1` `device.heartbeat` events is governed by the optional screen-to-device linkage PLY-124 defines (a screen row referencing an adopted `device_id`); corpus cases exercise `content.played` sourcing (unconditional, PLY-113) and status-report shape and cadence (unconditional, PLY-120–123), not `device.heartbeat` forwarding specifically.
