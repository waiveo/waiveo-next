...
| `table` | display | no | `source` (Binding, required, array), `columns` (non-empty array of `{headerMsg, cell}` or `{headerMsg, cellWidget, cellValue?}` — `cell` a BindingExpr and `cellWidget` a widget node, each evaluated per row under an implicit `item` scope, UIS-071/071a) | none | `rowPress?` (ActionRef, `item` in scope) |
