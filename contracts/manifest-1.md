...
- Out of scope: the pack ↔ host runtime protocol (`ctx/1`); the signature/trust envelope wrapping a sealed pack artifact and its distribution/marketplace semantics (a separate contract); the declarative UI grammar itself — page types, widgets, binding grammar (`ui-schema/1`); the automation rule vocabulary (`rules/1`); the device capability/class registry (a separate companion document); the on-the-wire representation of the verbs a manifest's declarations end up dispatching to (`ctx/1`).
