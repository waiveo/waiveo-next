...
**[SUR-001]** A surface MUST be mounted only from a pack whose artifact signature has verified against the platform's software-artifact trust bundle (`channel-index/1`) under the `first-party` trust channel; a pack under any other trust channel MUST NOT be offered a mount under this contract, regardless of any manifest declaration it carries.

*draft-note: the manifest/1 field a pack uses to declare that it ships a surface (and which surface entry point a mount resolves) is not yet specified in manifest/1 — this contract assumes such a field exists and resolves to exactly one bundled entry point per declared surface, and will cite it by requirement ID once manifest/1 defines it.*
...
**[SUR-006]** A mount's `mount_id` MUST be a ULID, unique for the lifetime of the issuing host page's own document, and MUST accompany every bridge message and every management-API call authenticated by that mount's `surface_token` as the correlated trace context (Bridge message envelope) — one value threading bridge traffic and the API calls it authorizes, consistent with the platform's general trace-propagation convention (`api/1` API-060–063).
...
**[SUR-042]** `expires_at` (SUR-002) MUST bound `surface_token`'s validity to no more than a short, fixed maximum from `issued_at`.

*draft-note: no normative source yet fixes the exact maximum; propose 10 minutes, chosen to keep a stolen token's exposure window short while remaining longer than `bridge.token_renew`'s (SUR-024) own round-trip — revisit once real numbers exist, consistent with this platform's other contracts' own timeout draft-notes.*
