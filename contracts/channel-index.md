...
**[CHI-020]** A channel's index document MUST be an array of artifact entries, each carrying at minimum `{artifact_id, kind, version, status, digest, size, download_url}` (Wire shapes, `ChannelIndex`). `kind` MUST be one of `platform-release` or `relay-release` (Channel namespaces), extensible by a companion contract per CHI-031. `status` MUST be one of `active`, `deprecated`, `yanked`.
...
- A marketplace pack or content-package's own index entry is now defined by `marketplace/1`, which reuses this same signed-index envelope (Scope); this corpus exercises only `platform-release` and `relay-release` entries.
