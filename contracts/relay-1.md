...
**[REL-015]** An enrolled relay MUST be able to renew its certificate in-band, over its existing authenticated connection, ahead of expiry and without presenting a fresh claim credential: `{csr}` in, `{cert, not_before, not_after}` out, under the same `relay_id` (REL-014). A renewed certificate takes effect at the relay's next connection attempt: issuing a renewal neither tears down nor re-keys an established connection — the connection continues under the certificate it authenticated with, and the superseded certificate, while never again eligible for re-enrollment (REL-021/REL-022), is not thereby revoked (REL-016).

*draft-note: until the in-band `renew`/`renew-ack` verb lands on the authenticated connection (an additive REL-004 minor; Wire shapes), a relay MAY drive ahead-of-expiry renewal over the Expired-certificate re-enrollment path's bootstrap exchange (REL-026–029) — eligibility there is issuance-record-based (REL-021), never expiry-gated, so a most-recently-issued, unrevoked certificate renews identically before or after its `not_after`. The renewal window — how far ahead of `not_after` a relay begins attempting renewal — is likewise not fixed by any normative source yet; proposed default: 30 days, jittered per relay.*
...
**[REL-061]** `screen_programs` MUST be an array of `{screen_id, program_revision, priority, display, content}` — `priority` one of `scheduled` or `preempt`, mirroring `player/1`'s own Lease `priority` field (`player/1` PLY-100); `display` one of `content` or `blank`, mirroring `player/1`'s own Lease `display` field (`player/1` PLY-093) — and `content` an array, in intended playback order, of signed content references `{asset_ref, url, expires_at}`, `asset_ref` a content-addressed `sha256:` URI in the same form `ctx/1`'s `assets` family uses. `content` MAY carry more than one item; each item is independently asset_ref-verifiable and independently fetchable — an N-item `content` array is N signed references, not just its first. This contract carries `content`, `priority`, and `display` alike opaquely from app peer to relay to screen; it does not define how a screen resolves or fetches content, sequences a multi-item `content` array, or adopts a priority class or display state, itself (`player/1`, Scope). Carrying `priority` here is what lets a `preempt`-priority assignment reach a relay's screen through its own offline-cached last-applied snapshot (Idempotent apply & enrollment-anchored trust) without requiring the relay's app-peer connection to be live at the moment a screen needs it; carrying `display` here is what lets a `blank` assignment reach a relay's screen the same way — riding the same offline-cached snapshot, so a blank assignment survives a WAN outage exactly as a preemption does.

**[REL-061a]** A `content` item MAY additionally carry `content_type` and `duration_ms`, each additive to this version's originally-published `{asset_ref, url, expires_at}` shape (REL-004). `content_type` names the item's kind, drawn from the same floor `player/1`'s own `content_types` capability declares (`player/1` PLY-014: at least `image`/`video`), and a relay carries it unmodified into the player/1 Lease content item's own `type` field for that entry; an item whose `content_type` is absent — an older feeder that predates this field — MUST be treated by a relay as `image`, this contract's own historical implicit value from before this field existed. `duration_ms`, when present and non-zero, is a per-item display-duration override, mirroring `data-model/1`'s own per-item `duration_seconds` override column (`data-model/1` DAT-042); this contract itself defines no consumer for it — it is carried opaquely, riding `hash`/`signature` (REL-053) like every other field, for a playback-advance mechanism outside this contract's scope to read.
...
**[REL-065]** `schedule` MUST carry the platform's scheduling-core data (playlist, cast, daypart, validity-window, fallback, preset-batch, and display-power rows) for this site, keyed by scope node. The cast rows are carried rather than pre-expanded into the slides they contain, because a relay re-resolves a screen's program from this section at every daypart boundary: a playlist item that references a cast the section does not carry is a reference the relay cannot dereference, and the screen would play a shorter rotation than the playlist states. This contract requires only that the section is present and travels under the same generation/hash/idempotent-apply rules as every other section; the row shapes themselves are that scheduling core's own content, not redefined here (their shapes defined by `data-model/1` Scheduling core).

**[REL-066]** `revocation_and_site` MUST carry `{revoked, site_effective}` and MAY additionally carry `content_origin`: `revoked` an array of opaque identifier strings the relay's player-credential authority (Player-credential authority) and connection-acceptance logic MUST treat as revoked even while disconnected from its app peer; `site_effective` a persisted copy of this site's `{tz, lat, long}` (mirroring `hello`'s `site_binding`, REL-036) so a relay's dayparting and sun/time evaluation remain correct across a restart without first completing a fresh `hello`; `content_origin`, when present, the base URL a screen fetches this site's content from — the app peer's own content origin — from which the relay constructs a schedule-resolved content item's `url` (`content_origin` + the content-addressed path for the item's `asset_ref`, the same `url` a `screen_programs[].content[]` entry carries per REL-061 and that a screen fetches directly, never through the relay per REL-140). The relay MUST NOT synthesize a relay-local content origin in its place: a snapshot that omits `content_origin` yields schedule-resolved content items whose `url` is empty (a degrade the screen surfaces as unresolvable content), never a fabricated one. `content_origin` is optional and structural-only — its absence is not a REL-060 completeness failure (the required section key is `revocation_and_site` itself, always present), and adding it preserves the REL-053 byte-identical-marshaling → `hash` invariant exactly as any other section field does.

**[REL-066a]** `revocation_and_site` MAY additionally carry `content_url_key`: a symmetric key, base64-encoded, with which a relay MINTS the signed content URLs REL-061 calls for. It is optional and structural-only on the same terms as `content_origin` (REL-066), and its absence yields unsigned URLs — the behaviour every deployment had before this field existed.

A relay needs this key because it CONSTRUCTS content URLs itself and no other party can do it for it. REL-066 requires the relay to derive a schedule-resolved item's `url` from the `content_origin` base; an app peer that pre-signed every URL could not, because it does not know which items a relay's own schedule resolution will select. REL-050 makes the same point more sharply: a relay applies its cached snapshot for the whole of an outage, so any URL carrying a deadline fixed at snapshot-build time eventually expires with nothing able to refresh it. Re-minting at the relay is therefore a correctness requirement for offline operation, not a convenience for schedule resolution.

**[REL-066d]** Where `content_url_key` is present, a relay MUST mint the `url` it serves to a screen at the moment it serves it, for EVERY content item alike — a schedule-resolved item it constructed and an app-authored item carried in `screen_programs` (REL-061) — rather than passing an app-authored `url` through unmodified.

Minting at serve time rather than at snapshot-build time is what makes the deadline meaningful under REL-050. A URL signed when the snapshot was built carries a deadline fixed at that instant, and a relay applying a cached snapshot through a long outage would serve deadlines that had already passed, with no way to refresh them while its app peer is unreachable. Minting from the relay's own clock, at the moment a screen asks, means an offline relay keeps serving fetchable content for as long as it keeps serving anything.

A consequence worth stating: an app peer's own `url` and `expires_at` in `screen_programs` are therefore inputs to the relay's minting, not the values a screen receives. An app peer MAY leave them at their unsigned form when it knows every relay at the site holds the key.

**[REL-066b]** A relay MUST treat `content_url_key` as credential material: it MUST NOT be logged, MUST NOT appear in telemetry, and MUST NOT be served to a screen — a screen receives minted URLs, never the key that mints them. A relay MUST use it only to mint URLs for content at its own site.

**[REL-066c]** This is deliberately a SYMMETRIC key shared with every relay enrolled to the site, and the consequences are stated here so a later reader knows they were chosen rather than overlooked. Any enrolled relay can mint a URL for any asset at its site, with any expiry it selects; there is no per-relay attribution in a minted URL; and revocation is key rotation, which invalidates every outstanding URL at that site at once. The trust this extends is close to what an enrolled relay already holds, since the snapshot delivers it every content digest at the site regardless.

The successor, when per-relay attribution or per-relay revocation is needed, is asymmetric: each relay signs with its own enrollment key and the origin verifies against the enrolled set, so one relay can be revoked without disturbing the others. That is a fleet-scale concern, and this field is the shape chosen until it becomes one.
...
**[REL-090a]** A telemetry entry MAY additionally carry `trace_id`: the Canonical ULID correlation id of the originating operation this entry records, exactly as REL-006 requires of a relay/1 message whose work traces to one — carried PER ENTRY, never per `telemetry.push`, because a push batches entries recorded independently across a whole disconnection window and a batch-level value would assert one causal chain across work that shares none. The app peer MUST propagate a valid value unmodified into the `trace_id` of the `events/1` envelope it reconstructs from the entry (`events/1` EVT-010), which is the identifier that makes an operation traceable from the management door through the relay to the durable event it eventually produces (`api/1` API-063). An entry omitting `trace_id`, or carrying a value that is not a Canonical ULID, MUST still be delivered: the app peer substitutes an identifier of its own so the envelope satisfies EVT-010's own type, and MUST NOT reject or drop the entry — a defect in correlation metadata may not become the durable-class loss REL-093 and REL-103 forbid.
...
**[REL-110a]** Each candidate MUST additionally carry `{driver, native_id, device_class, entities}` and MAY carry `name` — `driver` and `native_id` the two site-scoped members of the identity a `device_id` is scoped to (REL-153), `device_class` the class whose vocabulary governs commands to it (`device-class-registry/1` REG-052), `entities` an array of `{key, device_class, state}` naming the addressable objects the device exposes (`key` a device-native handle unique within its own device; `state` present only once the relay has observed one), and `name` the device's own self-reported name where discovery supplies one. `match` alone identifies no device — it is the declared pattern that produced the sighting, and many devices answer one pattern — so without these members the app peer receives a report it can neither list, address, nor adopt an individual device from. A candidate MUST NOT carry a `device_id`, `entity_id`, or any other platform row identifier: those are the app peer's to assign (REL-110b), and a relay is not a source of identity for rows in the app peer's own registry.

**[REL-110b]** Both peers MUST derive a candidate's `device_id` and each of its entities' `entity_id` deterministically from the identity REL-153 fixes — `(site, driver, native_id)` for a device and that tuple plus the entity's own `key` for an entity — using an identical derivation, and the derived values MUST be syntactically valid Canonical ULIDs (`data-model/1` DAT-005a). Deriving rather than assigning is what makes the rest of this section consistent: two relays serving one site that both observe one device derive the same `device_id` as REL-153 requires; a `device_id` survives a full-set re-report (REL-111) and a restart of either peer unchanged; the app peer accepts no identifier from an untrusted peer; and the relay can resolve a `device.command` (REL-112) addressed to an entity of a device it has discovered but that nobody has adopted, which it could not do for an identifier it was never told. The `site` input is the app peer's own authoritative `site_binding.scope_node` (REL-036), which the relay adopts from `hello-ack` — so neither peer asserts it to the other. REL-112's constraint on a dispatched `entity_id` is that it be a SINGLE, ALREADY-RESOLVED entity rather than a selector or a class filter; a derived `entity_id` of a reported candidate satisfies that constraint, and a relay MAY resolve and execute a command against one before any adoption record exists — which is how an operator identifies a device (blinks it, wakes it) as part of deciding whether to adopt it at all. Nothing in this requirement makes discovery equal adoption: the poll cadence and per-entity policy of REL-063 remain authored by the app peer and reach the relay only in signed desired state.

**[REL-110c]** A candidate carrying `name` MAY additionally carry `name_rank` (a REL-004 additive member, added in a minor after REL-110a): a token naming the KIND OF RECORD that authored `name`, one of `none`, `machine`, `model`, `friendly` — `friendly` a record whose label is a name somebody gave the device, `model` a factory or product string, `machine` a label the device derived mechanically (an id, a hostname form, or a lossy rewrite such as a truncated-and-suffixed instance name), `none` a name the relay has no opinion about. A relay that reports `name_rank` MUST report it on every candidate it reports a `name` for. `name_rank` describes the SOURCE only and MUST NOT be read as a statement about the string: a relay MUST NOT rank by the shape of the name, because one device commonly announces both a chosen name and a mechanical one and the mechanical one can be the longer or better-formed string. A consumer MUST treat an ABSENT `name_rank` as "the reporting peer does not rank names" and MUST NOT treat it as equivalent to `none`; a consumer meeting a token it does not recognize MUST treat it as the LOWEST rank it knows — a rank is a licence to refuse a differently-sourced name, and a value the consumer cannot interpret must never be honoured as one. A consumer that retains a candidate's `name` beyond the reporting relay's own process lifetime — a durable mirror of the reported set — MUST retain the reported `name_rank` with it, as a pair, and MUST NOT retain one without the other. The rank exists because a relay's own ranked merge lives in memory that is re-minted at every relay restart, so the first report after one carries whichever source the relay's first sweep reached; without the rank a consumer holding the previous answer can only ask whether the newly reported name is non-empty, and a mechanically derived label therefore displaces a chosen name and persists.

**[REL-110d]** A candidate MAY additionally carry `class_rank` (a REL-004 additive member, added in a minor after REL-110a, and deliberately a SEPARATE member from REL-110c's `name_rank` rather than an amendment to it): a token naming how strongly the record behind `device_class` asserts what the device IS, one of `none`, `feature`, `product` — `product` the device's own product service or a pattern a declaration named for that kind of device, `feature` a capability or fabric membership any kind of device may advertise, `none` nothing that recognised the device at all. A relay that reports `class_rank` MUST report it on EVERY candidate, unconditionally — `device_class` is required by REL-110a, so a peer that ranks classes always has one to rank, and an ABSENT `class_rank` therefore means "the reporting peer does not rank classes" and nothing else; a consumer MUST NOT treat it as equivalent to `none`. `class_rank` states AUTHORITY only and MUST NOT be read as stating how specific the class is: how much a class narrows a device's command vocabulary (`device-class-registry/1` REG-052) follows from the class token itself and is the consumer's own to derive, so a consumer ranking two statements about one device MUST compare `class_rank` FIRST — a device commonly advertises a broad capability of a kind it is not, and comparing specificity first reclassifies a thermostat that advertises a media capability as a media player. A `class_rank` a candidate carries alongside a `device_class` that names nothing — a generic "not yet recognized" default — MUST be read as `none` whatever token was reported, in both directions: a report that recognized nothing cannot be authoritative about nothing, and honouring such a rank would let the generic default outrank every real classification and then hold the device there. A consumer meeting a token it does not recognize MUST treat it as the LOWEST rank it knows, for REL-110c's reason.

A consumer that retains a candidate's `device_class` beyond the reporting relay's own process lifetime MUST retain the reported `class_rank` with it, as a pair, and MUST NOT retain one without the other. Two further obligations bind that consumer specifically, in opposite directions. It MUST NOT honour an absent `class_rank` as a licence to displace a class it holds at a recorded rank — REL-110c leaves the equivalent question open for the name, and this requirement does not leave it open for the class, because a wrongly changed class silently removes commands rather than merely mislabelling a device. And it MUST accept an EQUALLY ranked restatement rather than refusing it: breaking a tie by specificity is sound only for a consumer whose decision expires — the relay's within-process merge is such a consumer, and specificity there is what keeps a device whose better record was missing from one sweep from being reclassified — whereas a retained decision has no expiry, so a device that permanently stops advertising its more specific service would report the honest, less specific class on every later report and be refused on every one of them, with nothing able to correct it and no distinction available between that and a corrected declaration.

The rank exists because the relay's class pick is a function of which records one discovery sweep happened to hold: a device advertising several services is classified from the best of them, and a sweep that misses that one record reports a different class for an unchanged device, which a consumer comparing only class tokens takes as a genuine reclassification and persists.
...
**[REL-111a]** REL-111's keying clause is superseded by this requirement, which replaces it in full and leaves the rest of REL-111 (a report is a replace, not a delta) untouched. The replace MUST be keyed by the candidate's own device identity (REL-110a's `(driver, native_id)`), not by `match`, and MUST be scoped to the relay the report was received from — identified by its enrolled cryptographic identity (REL-150), never by a `relay_id` the message asserts. Keying by `match` cannot express a LAN where two devices answer one declared pattern: both would collapse into a single candidate and neither could be listed, commanded, or adopted individually. Scoping to the authenticated reporter is what keeps a full-set replace from being a delete of another relay's devices, and a device reported by more than one relay MUST remain present while ANY of them still reports it (REL-153).
...
**[REL-116]** A relay MAY report `discovery.engine_state`: an unsolicited upward frame stating what its PASSIVE discovery engine is currently set up to watch for. It carries, for each passive lane, whether that lane is running and how many live watches it holds after the current generation applied, together with how many `pack_match_patterns` (REL-064) the applied generation declared and how many of those declarations the relay could not deliver — separated by cause, at minimum: a declared match whose lane is not running on this deployment, a well-formed match form no lane implements yet, and an entry that failed to parse. Every count MUST be present even when zero, and each lane's running flag MUST be present alongside its count. A count omitted at zero is indistinguishable from a count the peer declined to send, which collapses "this relay is watching for nothing" — the single condition this frame exists to surface — into "this relay did not say"; and a zero count is unreadable without its lane flag, since no live mDNS watch because the deployment never bound multicast and no live mDNS watch because the generation declared no mDNS patterns are different conditions with different owners.

**[REL-117]** A relay that sends `discovery.engine_state` MUST send it both when a desired-state generation applies and on establishing a connection, re-stating its current engine state on the new connection. Apply-time alone is insufficient because REL-070 suppresses re-applying an unchanged generation: a relay whose declared patterns have not changed would never report again, and an app peer that restarted — or a connection that dropped — would hold no engine state at all for a relay that has been watching correctly throughout. This differs deliberately from an ACTIVITY report such as a scan's, where having nothing in flight is genuinely nothing to say; a watch set is current configuration, always true and always worth re-stating to a peer that has not heard it. A relay that has applied no generation yet MUST send nothing rather than a zeroed body, that body being indistinguishable from the watching-for-nothing alarm of REL-116.

**[REL-118]** An app peer that holds `discovery.engine_state` MUST attribute each report to the reporting relay's authenticated identity (REL-150), never to any `relay_id` the frame asserts, and MUST replace that relay's previously held state wholesale rather than merging members — a member absent from a newer report MUST NOT survive from an older one, the relay being the sole authority on its own current watch set. A relay that has never reported MUST be ABSENT from the app peer's view rather than represented with zero counts, for REL-116's reason stated from the reading side: minting zeroes for a silent relay raises the watching-for-nothing alarm on every peer that has simply not been told yet.

**[REL-119]** A relay MAY report `screen.status`: its FULL current view of every screen it knows anything about, as an unsolicited fire-and-forget frame. The app peer MUST take each report as REPLACING its prior view of that relay's screens, and MUST attribute it to the reporting relay's authenticated identity (REL-150), never to any identifier the body carries — a relay cannot name another relay's screens.

A full-set replace rather than a delta, for REL-111's reason: a delta stream needs both sides to agree about a starting point they cannot establish across a reconnect, and a screen that has DISAPPEARED from a relay's view — its session dropped, its program removed — has to be able to leave the app peer's view, which only a replace expresses. An EMPTY `screens` array is therefore a meaningful value, not an omission: it is what says "this relay knows of no screens", and it is the value that clears a stale view. A producer MUST send it rather than omitting the member or sending null, and an app peer MUST honour it.

Unacknowledged, unlike `pairing.redeemed` (REL-124d). A lost redemption loses a fact nobody can recompute; a lost status report loses nothing, because the next one carries the whole truth again — so acknowledging it would add a correlation table and a retry ledger to protect data with a seconds-long shelf life.

**[REL-119a]** Every duration a `screen.status` entry carries MUST be an AGE — milliseconds before the report was taken, measured on the reporting relay's own clock — never an absolute instant. An instant crossing between two machines is wrong by their skew, and an appliance that boots before its clock settles makes that skew arbitrarily large; an age computed and consumed as an age is wrong by the transit time alone. This is the same clock-trust posture Clock trust takes for the rest of this contract, applied to the one frame whose whole content is timing.

A never-observed age MUST be reported as `-1`, and an app peer MUST treat it as a distinct state rather than as a large age. Zero is a real answer — "just now" — so it cannot carry this meaning. The distinction is operational, not decorative: a screen that pulled once and stopped is broken, and a screen that has never pulled was probably never paired, and those are different problems with different first actions.

**[REL-119b]** A `screen.status` entry carries the platform's INTENT for a screen and the screen's own CONFIRMATION separately, and a consumer MUST NOT render the first as evidence of the second. The intent members describe the program the screen was last HANDED, or — for a screen that has never pulled — the one still waiting for it; the acknowledged members describe the Lease the screen last ACCEPTED (`player/1` PLY-091 `accepted: true`), and they alone are what a surface may render as what is on that wall. A player that refuses a program leaves the intent members describing the program it refused, so a console reading them alone reports a wall as showing something it is not — which is a defect this frame's shape exists to prevent rather than a hypothetical.

A refusal (PLY-091 `accepted: false`, with that clause's required `reason`) MUST be reported as a STANDING fact, cleared by the screen accepting something and by nothing else, and MUST be carried as a boolean rather than as an age. That is a compatibility requirement, not a modelling preference: a relay is expected to be upgraded at or before its app peer, so an app peer reading reports from an OLDER relay is the ordinary case (REL-004), and an absent numeric decodes to `0` — which as an age would read as "refused just now", making every screen behind a pre-upgrade relay appear to be refusing its program. An absent boolean decodes to `false`, which is the correct reading for a relay that cannot report this: no claim.

**[REL-127]** An app peer MAY send `discovery.scan`, a command asking one relay to run a single ACTIVE scan of what it can reach, answered by a correlated `discovery.scan_result`. It is a command rather than a member of signed desired state because a scan is a transient action with a result, not durable configuration — the durable half (which subnets are scannable, any schedule, the rate budget) belongs to desired state, and this frame carries only "scan now, optionally scoped".

The result MUST be an ACCEPTANCE and MUST NOT carry findings. A scan outlives the request/response exchange that started it, and its findings already have a path upward: the relay reports them through the ordinary `device.candidates` report (REL-110/111), exactly as passive sightings arrive. So the result says whether a scan STARTED, and progress is read from the relay's own scan-state report (REL-129), never from this reply.

A request arriving while a scan is already running MUST be answered `ok` with `started: false` and the running scan's identifier, not as an error. Asking a scanning relay to scan is a benign repeat — an operator double-click, a retried request — and reporting it as a failure would make a harmless race look like one. A refused result (`ok: false`) MUST carry an error object bearing an Error-taxonomy code; an `ok: false` with no error is not a conformant result, since a refusal an operator cannot read is indistinguishable from a fault in the transport.

**[REL-128]** A `discovery.scan`'s scope is REQUESTED, never trusted. A relay MUST refuse a requested subnet that is not permitted by the policy its signed desired state carries, and MUST refuse one that is not on an interface it actually has. An app peer naming an arbitrary CIDR MUST NOT be able to point a relay's probes at a network the operator never enabled — the signed state is the authority on what may be probed, and this frame is a request against it. A scan with no scope means the relay's own default scope, which is the only scope a relay can honour without being told about a network it cannot see.

A named active lane the relay has not implemented MUST be ignored rather than refused, under REL-004 additive tolerance: a newer app peer naming a lane this relay lacks still gets the lanes it has, and a scan that refuses outright because of one unknown name is a scan an upgrade silently disabled.

The scan's stated reason is DESCRIPTIVE and MUST NOT be an authorization input. Nothing may be permitted or refused on the strength of it, so a caller that mislabels its own scan mislabels a log line and reaches nothing else. It exists because a relay that logs every scan identically leaves an operator unable to answer "why is this thing probing my network every fifteen minutes", which is the question the label is for.

**[REL-129]** A relay MAY report `discovery.scan_status`, its scan engine's current state, as an unsolicited fire-and-forget frame reported on TRANSITION — a scan starting, a scan finishing — rather than on a timer. The state changes only at those moments, so a periodic re-send would carry nothing new.

An app peer's view of a scan MAY therefore be stale, and that staleness MUST NOT be resolved by inference. A relay that dies mid-scan sends no completion, and the honest consequence is that the app peer's last-known state stays `scanning` until that relay reports again: the app genuinely does not know whether the scan finished, and the connected-relay set already tells an operator whether the relay is there at all. A host MUST NOT substitute a timeout-derived completion for a report it never received — a fabricated "probably done" is the same lie an empty device list tells when nothing is discovering.

Scan state is not required to survive a restart of either peer. An app peer that has lost it MUST report having none rather than reconstructing one, for the reason above; "this relay has told us nothing about scanning" and "this relay is idle" are different facts and only the relay can assert the second.
...
**[REL-121b]** A pairing-grant record MAY additionally carry `relay_id`: the enrolled identity (Multi-relay identity, REL-150) of the ONE relay this grant may be redeemed at. A relay that resolves a `grant_selector` (REL-126) to a record whose `relay_id` is present and is not its own enrolled identity MUST refuse the redemption with the same typed rejection an unresolvable selector draws (`player/1` PLY-036's `PAIRING_CODE_INVALID`), MUST NOT mint any credential for it, and MUST NOT record it consumed — a relay that may not consume a grant has nothing to say about it, and a distinguishable refusal would turn every relay of a site into an oracle for which grants exist at its siblings. The comparison MUST be against the relay's OWN enrolled identity — the identity its enrollment-issued certificate carries and its app peer authenticates it by (REL-041/150) — never a value a redeeming player supplies. A record whose `relay_id` is absent is unbound and remains redeemable by any relay the snapshot reached, which is this field's pre-existing behaviour and why REL-121c constrains who may mint one.

**[REL-121c]** An app peer MUST bind every one-time pairing grant it mints (`redemption_mode: one-time`, REL-121) to exactly one relay under REL-121b, and MUST NOT deliver an unbound one-time grant in `pairing_grants` (REL-067). One-time consumption is enforced by the redeeming relay alone: REL-122 makes a grant redeemable for the whole of its `ttl` with no app peer reachable, so no relay can consult any other party before consuming one, and `pairing_grants` is a section of the ONE signed snapshot every relay enrolled to the site applies (REL-050/067). An unbound one-time grant delivered to N relays is therefore redeemable N times — once per relay, each relay's own count correctly never exceeding one — and because a `pairing` grant is screen-bound (REL-121a), every one of those redemptions resolves to the SAME screen row and is served that screen's program (REL-061). Binding is what makes each relay's own local check-and-consume a site-wide one: exactly one relay can ever consume the grant, so that relay's count IS the site's. Binding aligns the grant with the code that selects it: a pairing code already encodes one relay's dial address (REL-126), so a code formed for one relay was never redeemable at another by any player that follows it, while the grant it selected was redeemable at all of them. That mismatch is what this closes. Two consequences MUST be stated rather than left implicit. First, REL-126 constrains the code to a dial address and does NOT require that address to resolve to exactly one relay — a deployment fronting several relays behind one name, VIP or NAT address is conformant, and for it binding is a real restriction rather than a formality: a code formed against such an address is redeemable only at the relay the grant names. Second, because the app peer holds no model of which relay a screen belongs to, an issuing surface serving a site with more than one connected relay MUST let the caller name the relay a grant is bound to rather than choosing one on the caller's behalf; a chosen-for-you binding surfaces at redemption as an ordinary invalid-code refusal (REL-121b), which is deliberately indistinguishable from a mistyped code and so gives an operator nothing to act on.
...
**[REL-124a]** REL-124's report MUST ride this contract's own connection as `pairing.redeemed {grant_id, redeemed_at}`, acknowledged by the app peer with `pairing.redeemed_ack {grant_id}` echoing the report's own correlation id (REL-006), not the telemetry-upstream channel. That channel has no vehicle for it: REL-095 closes its schema set to exactly the five `events/1` registered schemas REL-093–094 name and makes `events/1` their sole normative source, so relay/1 neither has a schema there that carries a redemption nor may mint one — doing so would be this contract editing another contract's registry. REL-095 does not say that in a prohibitive verb; it makes `events/1` the sole normative source and closes this channel's set to the five named schemas, and the prohibition follows from REL-090, which admits only a `schema` drawn from that closed set. The distinction is recorded because a citation that overstates its source is one a later reader either takes on trust or discovers is wrong, and both are worse than the accurate chain. REL-124's own "at the next telemetry or connection opportunity" already admits the connection as a vehicle; this requirement fixes it as THE vehicle.

**[REL-124d]** A relay MUST NOT discard its record that a redemption is owed until that redemption's own `pairing.redeemed_ack` has been received — the same discipline REL-092 already imposes on the telemetry channel, and for the same reason: a write that left the socket is not a record the app peer kept, and a report retired on the strength of a successful write alone is lost precisely when the app peer failed to record it. An unacknowledged redemption MUST be re-sent at each subsequent connection opportunity, so a redemption performed while disconnected (REL-122) — including one performed before a restart, which is why REL-142a admits the owed-report ledger to durable storage — still reaches the app peer. The one exception is a report the app peer refuses `PAIRING_REPORT_UNAUTHORIZED` (REL-124b): the Error taxonomy marks that refusal non-retryable, so re-sending it would repeat the identical refusal forever, and the relay MUST stop re-sending that report rather than wedge its ledger behind one the app peer will never take.

**[REL-124b]** An app peer receiving `pairing.redeemed` MUST attribute it to the connection's own authenticated identity (REL-041/150), never to a `relay_id` the frame asserts, and MUST refuse a report naming a grant bound (REL-121b) to a different relay (`PAIRING_REPORT_UNAUTHORIZED`, Error taxonomy) rather than acknowledging it. The report MUST be consumption-only and idempotent: it may mark a grant consumed and retire it from later generations, and MUST NOT un-consume one, revive an expired or already-retired one, create a grant record that did not exist, or alter any state belonging to another relay. A report naming a grant the app peer no longer holds MUST be acknowledged as a no-op rather than refused — a relay that reports a redemption twice, or reports one the app peer already retired, is behaving exactly as REL-124d's re-send rule requires. An app peer that cannot record a report it does hold MUST answer a typed error rather than an ack, so the relay keeps the redemption owed (REL-124d) instead of retiring one nothing recorded. A relay is untrusted input on this verb precisely as it is on `device.candidates` (REL-111a).

**[REL-124c]** REL-124's report is an audit and retirement mechanism, NOT the enforcement of one-time redemption: the site-wide at-most-once property is REL-121b/REL-121c's binding, which holds with no report ever delivered and no app peer ever reachable. An app peer MUST NOT treat the absence of a report as evidence a grant was not redeemed, and MUST NOT rely on removing a reported grant from a later generation to prevent a second redemption — removal always trails the redemption it reflects, by the reporting relay's own disconnection (REL-122) and by every other relay's interval between generations (REL-057), so a rule that depended on it would be a rule with a window.
...
**[REL-132]** The clock floor MUST advance only from a time value the relay can verify independent of an unauthenticated claim — concretely, either a timestamp signed under the same enrollment-anchored key the relay already verifies desired state against (`desired_state_verification_key`, REL-012, REL-071/072), or an external time source whose own authentication the relay can verify (for instance, an authenticated network-time protocol). An app peer's `clock.hint` (REL-133), carrying no such verifiable signature, MUST NOT by itself advance the floor. The timezone and coordinates the relay applies that verified time against for sun and time evaluation are site-placement data carried in `site_binding`/`site_effective` (REL-036, REL-066), not themselves a source of wall-clock time.
...
**[REL-137]** The trust anchor established at enrollment that REL-136 and REL-003 match the app peer's presented certificate against MUST be the app peer's enrolled leaf public key itself — a SubjectPublicKeyInfo pin (the `trust_pin` datum REL-011 delivers), compared key-for-key — never chain validation that would accept any leaf issued under a broader certificate authority the enrollment happened to learn. Pinning the enrolled leaf key is what keeps identity bound to the exact key across REL-136's skew-tolerant or deferred temporal check: a leaf that has since rotated off, even one still chaining to the same authority, does not satisfy the pin. This is the same enrollment-anchored, key-specific trust the connection relies on in the other direction — where the app peer verifies `channel_binding_signature` against the relay's own enrollment-learned key (REL-032, REL-041) and re-enrollment proves possession of that exact key (REL-027–029) — so a rotation of the app peer's own key requires re-anchoring at enrollment (REL-017), never a silent swap under an unchanged authority.
...
**[REL-142a]** REL-142's list additionally admits the player-credential state this contract's own Player-credential authority section requires a relay to keep across a restart: which one-time pairing grants it has consumed (REL-121 — a redemption count that never exceeds one is not suspended by a power cycle) and which of those consumptions it has not yet reported upstream (REL-124/REL-124a — a report owed at "the next connection opportunity" survives a restart that happens before one arrives). Both are small, per-redemption records of the relay's own decisions — an identifier and a timestamp each, no asset bytes, no content store, nothing Gateway posture (REL-140/141) excludes — and the same class of relay-owned, player/1-defined credential state `player/1` PLY-091/PLY-105 already mirror into this tier.
...
// StateSnapshot (app peer -> relay — every sections key MUST be present, REL-060)
{
  ...
  "body": {
    ...
    "sections": {
      ...
      "schedule": { "scope_nodes": [], "playlists": [], "casts": [], "schedules": [], "validity_windows": [], "dayparts": [], "fallbacks": [], "preset_batches": [] },
...
```json
// PairingRedeemed (relay -> app peer — REL-124/124a; re-sent until acknowledged, REL-124d)
{ "type": "pairing.redeemed", "id": "01J8Z4K4N5P6Q7R8S9T0V1W3C0", "relay_id": "01J8Z4K4N5P6Q7R8S9T0V1W3A1", "body": { "grant_id": "01J8Z4K4N5P6Q7R8S9T0V1W3C1", "redeemed_at": 1752537600000 } }
```

```json
// PairingRedeemedAck (app peer -> relay — echoes the report's correlation id, REL-006/124a)
{ "type": "pairing.redeemed_ack", "id": "01J8Z4K4N5P6Q7R8S9T0V1W3C0", "relay_id": "01J8Z4K4N5P6Q7R8S9T0V1W3A1", "body": { "grant_id": "01J8Z4K4N5P6Q7R8S9T0V1W3C1" } }
```

```json
// A report the app peer refuses because the grant is bound to another relay (REL-124b) — never acknowledged
{ "type": "error", "id": "01J8Z4K4N5P6Q7R8S9T0V1W3C0", "relay_id": "01J8Z4K4N5P6Q7R8S9T0V1W3A1", "code": "PAIRING_REPORT_UNAUTHORIZED", "message": "the reported pairing grant is bound to a different relay" }
```
...
| `PAIRING_REPORT_UNAUTHORIZED` | A `pairing.redeemed` report named a pairing grant bound to a relay other than the reporting connection's own authenticated identity (REL-124b). | no |
...
- The platform's scheduling-core row shapes (`schedule`, REL-065) are normatively defined by `data-model/1` (its Scheduling core); corpus cases exercising snapshot completeness (REL-060) treat this section as an opaque, structurally-present value, deferring its row-shape validation to `data-model/1`'s own corpus.
