...
**[REL-124a]** REL-124's report MUST ride this contract's own connection as `pairing.redeemed {grant_id, redeemed_at}`, acknowledged by the app peer with `pairing.redeemed_ack {grant_id}` echoing the report's own correlation id (REL-006), not the telemetry-upstream channel. That channel has no vehicle for it: REL-095 closes its schema set to exactly the five `events/1` registered schemas REL-093–094 name and makes `events/1` their sole normative source, so relay/1 neither has a schema there that carries a redemption nor may mint one — doing so would be this contract editing another contract's registry, which REL-095 forbids in terms. REL-124's own "at the next telemetry or connection opportunity" already admits the connection as a vehicle; this requirement fixes it as THE vehicle.
