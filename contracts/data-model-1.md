...
**[DAT-041]** `items` MUST be an array (possibly empty) of `{source, asset_ref?, content_type?, pack_id?, content_id?, slide?, cast_id?, duration_seconds?}`, in play order. `source` MUST be exactly one of `asset`, `playable`, `slide`, or `cast`. When `source` is `asset`, `asset_ref` MUST be present (a content-addressed `sha256:` URI, the same form `player/1`'s own Content reference uses, `player/1` PLY-083); when `source` is `playable`, `pack_id` and `content_id` MUST both be present, `content_id` naming one pack's own `contributes.playable` entry (`manifest/1` MAN-080); when `source` is `slide`, `slide` MUST be present and MUST carry the ordered layer stack the item's content IS; when `source` is `cast`, `cast_id` MUST be present and MUST reference an existing Cast row's own `id` (DAT-043, `REFERENCE_INVALID`). An item MUST declare exactly one of those four content shapes — never two, and never none.

An inline `slide`'s `layers` MUST satisfy exactly the rules a cast slide's `layers` do (DAT-043) and MUST be rejected when they do not (`PLAYLIST_ITEM_SLIDE_LAYERS_INVALID`, Field-level error register). It is one rule stated for two shapes rather than two rules, because everything downstream of authoring already treats them as one: both are projected onto delivered content through the identical layer resolution, both have their content references held against the retention sweep, and both are reported to the off-appliance rasterizer as outstanding work. An authoring gate that read only the cast shape therefore did not make the inline shape more permissive — it made it UNVALIDATED, so a stack refused with a 422 in a cast was accepted with a 201 inline and every consumer downstream inherited a shape the surface promises cannot exist.
...
| `PLAYLIST_ITEM_SLIDE_LAYERS_INVALID` | A `source: "slide"` playlist item's inline `slide.layers` stack is empty or violates the layer shape's own rules — an unknown kind, a zero-area or off-canvas geometry, a missing per-kind required field, a `derive` layer with no spec (DAT-041). The same rule `CAST_SLIDE_LAYERS_INVALID` states for a cast slide, under its own code because the field an operator must go and fix is `items[i].slide.layers`, not a slide of a cast they may not even have. | no — fix the named item's layers |
