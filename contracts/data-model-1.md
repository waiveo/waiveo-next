...
- In scope: the scope-node tree's `kind` vocabulary, parent/child hierarchy, and referential-integrity rules; the resource-row column baseline every row this contract defines carries; the org node's `account_state` and entitlements-document slot, and the self-hosted one-org/one-site invariant; the `tz`/`lat`/`long` columns and their site-default/override resolution rule; the playlist, schedule, daypart, validity-window, fallback, and preset-batch row schemas; the platform-ownership invariant barring a pack-owned scheduler row; the display-power dimension and its projection onto `player/1`'s playback lease; the schedule-side `misfire` default per row kind.
...
**[DAT-050]** A schedule row MUST be `{id, scope_node, name, fallback_id?, misfire?, external_id?, labels?, revision, created_at, updated_at}` (Schedule, Wire shapes) — the Resource-row baseline (DAT-005–008) plus `name`, an optional `fallback_id` (referencing a Fallback row's own `id`), and an optional `misfire` (Schedule-side misfire default).

**[DAT-051]** A schedule row governs its own `scope_node` only; this contract defines no inheritance or composition rule for a schedule attached at one scope node against a second schedule attached at an ancestor or descendant node.

*draft-note: whether two schedule rows bound to nodes on the same ancestor chain (for instance, a `site`-level schedule and a `screen`-level schedule covering the same screen) compose, override, or conflict is not defined by this contract. Time as data's site-default/screen-override cascade (DAT-033) is specific to the `tz`/`lat`/`long` columns and is not, without further specification, generalized to schedule attachment.*
...
**[DAT-073]** Two daypart rows belonging to the same `schedule_id` MUST NOT declare an overlapping `(days_of_week, start_time, end_time)` range — a create or update request that would produce an overlap MUST be rejected (`DAYPART_OVERLAP`, Error taxonomy); a schedule's dayparts partition time, they do not layer over one another.
...
**[DAT-081]** A fallback row is what a `scope_node` resolves to whenever no schedule in force at it (DAT-052) has a daypart currently holding (DAT-070–073) — never a fetch-failure substitute for content a player already holds but cannot refresh, which is `player/1`'s own, distinct, player-local concern (`player/1` PLY-087).
...
**[DAT-110]** A scope node's currently holding daypart — the one whose `days_of_week`/`start_time`/`end_time` (DAT-071–072) contains the current local instant, drawn from the schedule(s) in force at it (DAT-052) — MUST be evaluated relay-side, against that scope node's own effective `tz` (Time as data, DAT-033), exactly as `rules/1`'s own time-based and sun-based evaluation is (`rules/1` RUL-340, RUL-060). This is the scheduling-core content `relay/1`'s own `schedule` desired-state section carries, keyed by scope node (`relay/1` REL-065).

**[DAT-111]** When two or more schedules in force at the same scope node each have a currently holding daypart, this contract does not itself define a resolution order between them.

*draft-note: DAT-111 is a real gap this contract leaves open, not an oversight — it follows directly from DAT-051's own open question (whether multiple schedules at related scope nodes compose at all). A future minor should either forbid the situation outright at validation time or define an explicit precedence rule.*
...
- Corpus: `conformance/corpora/data-model-1/` — seed cases covering the scope-node tree and its referential-integrity/attachment rules, the org node's `account_state` vocabulary, the `tz`/`lat`/`long` override-over-default resolution rule, a `display_power: off` daypart and its Lease projection, a preset-batch partial-failure outcome, the schedule-side `misfire` default by row kind, and a rejected pack-owned scheduler row.
- DAT-111's cross-schedule resolution order and DAT-051's schedule-composition question are both open by design (their own draft-notes) and so carry no corpus case — a case cannot exercise a rule this contract does not yet state.
