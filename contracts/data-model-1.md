...
**[DAT-004a]** A screen identity row (DAT-004) is the row a `screen_id` names, and a device row is the row a `device_id` names: the `screen_id` carried by a `relay/1` `screen_programs` entry (`relay/1` REL-061), learned by a player at pairing redemption (`player/1` PLY-035), referenced by a screen's optional adopted-device link (`player/1` PLY-124), and recorded on an `events/1` `content.played` payload (`events/1` EVT-050) MUST each be the `id` of one screen identity row; the `device_id` carried by a `relay/1` `device_inventory` entry (`relay/1` REL-063) MUST be the `id` of one device row. Neither identifier is a scope node's own `id`. A screen identity row and the scope node it is placed under are distinct rows with distinct identifiers, and a `screen`-kind scope node is a placement classification (DAT-001) — never a screen identity in its own right; a `screen_id` that resolves to a scope node rather than to a screen identity row MUST be rejected (`SCREEN_ID_UNRESOLVED`, Error taxonomy) rather than silently accepted as the node's own id.
