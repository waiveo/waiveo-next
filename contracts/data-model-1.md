...
**[DAT-073]** Two daypart rows belonging to the same `schedule_id` MUST NOT overlap — a create or update request that would produce an overlap MUST be rejected (`DAYPART_OVERLAP`, Error taxonomy). Two dayparts overlap when their `days_of_week` intersect **and** their `[start_time, end_time)` local time-of-day intervals intersect — each interval evaluated as a half-open range (its `end_time` boundary exclusive) in the schedule's own effective `tz` (all of a schedule's dayparts share one `scope_node`, DAT-007, hence one effective `tz`, DAT-033), and expanded across the following calendar day wherever a window wraps past local midnight (DAT-072). A schedule's dayparts thus partition time, they do not layer over one another.
