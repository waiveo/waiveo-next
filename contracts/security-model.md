...
**[SEC-062]** `recover` MUST report the app's own current clock-accuracy assessment before proceeding with any other recovery step, so an apparent TOTP lockout caused by clock skew — TOTP codes are time-windowed — is diagnosable before the operator assumes a lost secret.

*draft-note: the app-side clock-trust floor's own persistence and update mechanics (analogous to `relay/1`'s clock-trust exchange, REL-130–136) are not yet defined by any contract in this corpus; SEC-062 requires that `recover` surface whatever clock-accuracy signal the app maintains, without itself specifying how that signal is computed or persisted — a gap for whichever contract or companion document ends up owning app-boot/clock concerns.*
...
**[SEC-075]** The console binding's verb set MUST be limited to: grant issuance and redemption (Grants), session/API-key revocation (SEC-020), read-only service status, `restore-from-snapshot` (`archive/1`, referenced), and a clock-floor reset (relay-analogous, cross-ref `relay/1` Clock trust) — general resource data access MUST NOT be exposed over this binding, even to a caller who has already proven uid-0. A request naming any other verb MUST be refused with `CONSOLE_VERB_NOT_ALLOWED` (Error taxonomy).
...
**[SEC-121]** Factory reset on the one-box self-hosted topology MUST destroy all local key material: the workspace and its data key (SEC-040), the box key (SEC-041), and the relay's device identity plus its enrollment certificate/keypair (`relay/1` Enrollment). This MUST force fresh enrollment on every principal and MUST re-open the first-boot claim window (SEC-120).
...
*draft-note: elsewhere in the platform's own design material, every product-feature pack is described as optionally, not automatically, installed — which leaves `system` as the only pack this contract can defensibly mark even a tier-status *candidate*, and even that is inference (system hosts the variables/secrets surface, described independently of the optionally-installed product-feature packs) rather than a settled assignment. This table's "Tier status" column is this contract's own best-effort scaffold pending a firmer decision on the required-pack roster; the capability/egress/device-plane/surface columns are populated for the full named pack set regardless of eventual tier status, since the widening-consent rule (SEC-131) needs a baseline the moment any deployment does grant one of these tier status.*
