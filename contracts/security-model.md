...
**[SEC-003f]** The revocation operation (`api/1` API-140) MUST require `owner` at the workspace root. Not `admin`: the blast radius exceeds SEC-020's credential revocation, which ends one principal's sessions — revoking a site's only relay certificate takes every screen at that site dark, and the act is terminal (`api/1` API-142). An authorization floor is set by what the act can do at its worst, not by how often it is reached.
...
**[SEC-012a]** An issuer MUST NOT be permitted to issue a credential-reset grant (Credential-reset grants) for a target principal whose effective role is strictly above the issuer's own. SEC-012 permits `admin` to issue "for any `user` principal", and an `owner` is a `user` principal holding a password credential — so without this restriction the flow is a path from `admin` to `owner` in three calls: issue (the one-time code is returned to the issuing admin by SEC-050's own design), redeem (the redemption surface is necessarily reachable without an existing credential, since its caller is by definition someone who cannot sign in), then authenticate as the target. Every power SEC-011 reserves to `owner` would then be reachable by a role this contract deliberately withheld them from, and SEC-011's guarantee that a deployment always retains an `owner` would be satisfied by an `owner` an admin had taken over. The comparison is against the issuer's *effective* role (SEC-010's highest binding), and it is STRICTLY above rather than at-or-above: a peer-level reset grants the issuer no authority they did not already hold, while an upward one grants authority the platform refused them. A refused issuance MUST emit its own `audit.event` (SEC-034/EVT-083) — an attempt to reset a principal who outranks the issuer is an escalation attempt and is precisely the record an operator needs afterwards. This restriction binds the credential-exchange surfaces only: a redemption admitted over the console binding is exempt, because SEC-076 admits that binding's verbs on the ground that uid-0 "could already perform an equivalent action through direct filesystem or OS-level access on the same host", so a role floor there constrains nobody it does not already trust.
...
**[SEC-037]** A `tier-grant` purpose grant mints the `pack-service` principal an installed pack's process runs as — the ceremony SEC-003a names for that kind, and the reason an api-key for one is refused. The grant MUST be `one-time`, MUST carry `resulting_principal_kind: pack-service`, and SHOULD use a `ttl` on the order of a minute: it is redeemed by a process the host has just started, not by a human who must be given time to type it.

The host MUST hand the code to the pack process over a channel that does not outlive the start (its standard input, or a file it consumes and unlinks), and MUST NOT pass it in an environment variable. An environment variable is readable by anything that can read the process table for the life of the process, which would make a one-time short-lived code behave like a long-lived credential — defeating the only two properties that make handing it over safe at all.

The pack process exchanges the code for its own credential; the code is spent by that exchange. A leaked code is therefore already useless, which is the property that lets the host start a pack without ever holding a long-lived secret on the pack's behalf.

*draft-note: the ROLE a tier grant confers is deliberately the issuer's choice rather than fixed here — "tier" is the pack's trust channel (`marketplace/1`), and the mapping from channel to authority is deployment policy. What this contract fixes is the ceremony, not the policy. Note also that no per-pack authorization exists in the request path today (there is deliberately no pack-specific rule on pack-data writes), so a pack-service principal's role is currently its whole authority — including over other packs' collections.*
...
**[SEC-054]** A principal holding a `password` credential MUST be able to change it **itself**, presenting its CURRENT password in the same request as the new one. Requiring the current password is what distinguishes this from SEC-050's admin-issued reset and is not optional: without it a stolen session is a permanent account takeover rather than a bounded one, because the thief could lock the owner out of their own principal using nothing but the session they already hold. The operation MUST change only the `password` credential of the **calling** principal — never another principal's (that is SEC-050's flow, and it exists precisely so that changing someone else's credential is an act with an audit trail naming an admin), and never the caller's TOTP enrollment, for the same reason SEC-052 withholds TOTP from a credential-reset grant. On success a host MUST revoke the caller's OTHER sessions, and MUST NOT revoke the one the change was made from: an operator rotating a credential they believe is exposed is not helped by a change that leaves every other session live, and is actively harmed by one that signs them out mid-task and hands them a fresh login with a password they may have mistyped twice. It MUST NOT revoke the caller's api-key credentials. That is the deliberate asymmetry with SEC-053, whose blanket revocation is right for a RESET — where the evidence is that nobody could prove knowledge of the password — and wrong here, where the caller just did: silently killing every running integration on a voluntary rotation is a consequence no operator expects from a password field, and each key is separately listed and revocable by someone who does want that. A change presented with an incorrect current password MUST be refused and MUST be subject to the same attempt budget SEC-090 applies to a sign-in, since it is a password guess by another name.
