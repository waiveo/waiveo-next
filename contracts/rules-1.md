...
**[RUL-301]** State and numeric triggers MUST NOT dispatch their rule's actions until a **stabilization window** has elapsed after the evaluating engine reaches a defined readiness point (an engine restart, entity enrollment, or generation-apply, as applicable). The window is applied per qualifying trigger, at the same (trigger, entity) granularity as first-observation state (RUL-304): each trigger a given readiness point makes newly eligible — a trigger new or changed in an applied generation (RUL-303), a trigger watching a newly enrolled entity, or every trigger across an engine restart — serves its own window from that point, rather than one engine-wide gate deferring every trigger against a single shared readiness signal. The window MUST be bounded, config-visible, and fallback-timed — a maximum wait applies even if the readiness signal the window is keyed to never arrives, so the gate cannot wedge open indefinitely.
...
**[RUL-330]** Every entity observation carries two independent facts: whether the canonical state string changed, and whether any attribute's value changed. A `state` trigger or `state` condition's fire/pass behavior with respect to this distinction is governed exactly by RUL-023's four-way rule (attribute-scoped bounded/unbounded, state-scoped, unscoped) — restated here as the general principle: **state-scoped** matching (`attribute` absent, `from`/`to` present) MUST NOT be satisfied by an attribute-only change, regardless of what the unchanged state string equals; **unscoped** matching (`attribute`, `from`, `to` all absent) MUST be satisfied by either kind of change. Which attribute changes constitute the aggregate any-attribute-changed fact this section turns on is not defined here in isolation: it is governed by each attribute's `change_emission` in the device-class registry (`device-class-registry/1` REG-044), which this contract reads as the canonical source — a `significant` attribute's value change contributes to that fact, a `cosmetic` attribute's does not. That classification gates only this aggregate signal; an attribute-scoped match naming an attribute by name (RUL-022/RUL-023) still evaluates that attribute's own transition regardless of its `change_emission` (`device-class-registry/1` REG-045).
...
**[RUL-394]** A host MUST retain an automation's **previous definitions** and MUST be able to restore one, so that an operator who breaks a working rule can put back the one that worked. Every successful update to an automation MUST record the definition it REPLACED, in the same transaction as the write — a version captured outside it is a version that can be missing for the one write that mattered — and each retained version MUST carry the revision it was superseded at, so the sequence an operator reads is the sequence that happened. Restoring MUST write the restored definition as a NEW update, not as a rewind: the restore itself is therefore recorded like any other change, the version history only ever grows, and an operator who restores the wrong version can restore back. A host MUST NOT let a restore resurrect a deleted automation — the versions of a deleted rule go with it, on the same terms its own rows do — and MUST NOT let one alter anything the definition does not contain: a restore returns the rule's *authored content* to an earlier state, and MUST NOT change whether the rule is enabled, because an operator restoring the logic of a rule they disabled while debugging it does not thereby ask for it to start firing. A restored definition MUST be re-validated exactly as an authored one is (RUL-020's compile gate): a version that compiled under an earlier build of this platform is not thereby valid under this one, and restoring it without checking would place a rule the evaluator cannot run into a slot the console reports as healthy. Retention MAY be bounded, and where it is bounded the OLDEST versions MUST be discarded first and the bound MUST be stated to the operator rather than left to be discovered as a history that silently stops.
...
| `SIGNAGE_CONTENT_AMBIGUOUS` | A signage action does not declare exactly one thing to show: a `play_cast` with no `cast_id` (RUL-234), or a `show_alert` declaring both `cast_id` and `message`, or neither (RUL-235). | no |
| `MEMBER_TYPE_INVALID` | A member is present but not of the type this contract declares for it: an `EntityRef`'s `entity_id` that is not a canonical ULID (RUL-010), or a signage action's `screen_id`/`cast_id`/`message`/`ttl_seconds` written at any other type (RUL-233/RUL-234/RUL-235). | no |
