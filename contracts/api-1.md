...
**[API-150]** api/1 exposes a **restart** operation that stops the application-server process so a supervisor starts a replacement. Its subject is the process, not the host: it MUST NOT reboot the machine, power-cycle it, or restart any unit other than the application server's own. A restart of the app server takes the console and the api/1 surface away for seconds; a host reboot takes the relay, the content origin and every LAN service with it, and the two MUST NOT be reachable through one control.

The operation exists because the platform already instructs operators to perform it — the first-boot claim's own refusals tell an operator to restart the box to be presented a fresh setup code, and a staged restore reports "restart to finish" — and because installing or updating a code-carrying extension is permitted to require one. A product that tells an operator to do something it gives them no way to do has published a dead end, not an instruction.

**[API-151]** The operation's role floor MUST be `owner` at the workspace root — the same floor the workspace export (API-120–121) and the service-log read (API-132) carry. A restart is workspace-wide and not scope-narrowable: there is no node to narrow it to, so admitting an `admin` bound at one site would let a principal with authority over one site take the whole deployment's surface down.

**[API-152]** A successful response MUST be `202 Accepted` and MUST NOT claim the restart happened. The process is still running when the response is written — it must be, or the bytes could not be sent — so the only true statement a 2xx can carry is that the request was accepted. A `200` reading "restarted" would be a claim about a future the responding process cannot observe, and the one case where it is false (the stop refuses, the supervisor never starts a replacement) is the case an operator most needs to be told about.

**[API-153]** The acceptance body MUST publish what the client can actually observe: the instant the request was accepted, how long the process will go on serving before it stops, and **an identifier of the CURRENT process instance that a later read of this same surface returns** — so a client learns the restart completed by observing a DIFFERENT instance answering, never by assuming one did. Without that identifier a client can only poll for reachability, which cannot distinguish "came back" from "never went".

**[API-154]** A deployment whose process is not supervised — nothing is configured to start a replacement when this one exits — MUST refuse with `RESTART_UNSUPPORTED`, and MUST NOT stop. This is a platform-capability refusal, not a temporary one: `UNAVAILABLE` would invite a retry that an unsupervised host will never satisfy. Whether a supervisor exists MUST be a property the deployment DECLARES, never one the process infers from its environment. An inference that is wrong in the permissive direction — a supervisor detected, configured not to restart — is an endpoint that kills the process and never brings it back, which is the exact failure this refusal exists to prevent; fail-closed is the only safe default, and the refusal MUST name what to declare.

**[API-155]** A second restart request, arriving after one has been accepted and before the process stops, MUST refuse with `RESTART_IN_PROGRESS` rather than accepting a second one. Two accepted restarts cannot produce two restarts — the first stop is the only one that can happen — so a second `202` would report an act that will not occur. A retry of the SAME request under the Idempotency-Key convention (Idempotency-Key) is not a second request and MUST replay the original acceptance.

**[API-156]** A restart MUST refuse with `RESTART_BLOCKED` while accepted work is in flight that a restart cannot be resumed from — work whose per-target progress is durable but whose operation is not re-appliable (API-116). Such an operation's stranded targets are reconciled to `failed` on the next boot rather than resumed, so restarting through one converts work that was running into work that failed, and for an irreversible ordered sequence (a workspace delete, SEC-121) it can leave a half-applied state no later read can describe. The refusal MUST name the in-flight work, so the operator waits for a thing they can see rather than retrying blind.

**[API-157]** A restart MUST run the same graceful shutdown the platform's ordinary termination signal runs — connections drained, accepted work drained on the same budget, durable state closed — and MUST emit an `audit.event` (`events/1` EVT-081) attributing it to the invoking principal, both for the accepted case and for every refusal above (EVT-083). A privileged operation that removes the surface every other operation is audited through is the last record written before the gap in the trail, and it is the one that explains it.
...
| `RESTART_UNSUPPORTED` | The restart operation (Application-server restart) was called on a deployment that has not declared a supervisor, so nothing would start a replacement process. A capability refusal, not a temporary one, for the same reason `LOG_SOURCE_UNAVAILABLE` is. | no — declare a supervisor on the unit that starts this process |
| `RESTART_IN_PROGRESS` | A restart has already been accepted on this process and has not yet stopped it (API-155). | no — the restart already accepted is the one that will happen |
| `RESTART_BLOCKED` | Accepted work is in flight that a restart cannot be resumed from (API-156); the named work must reach a terminal state first. | yes — after the named work finishes |
| `ARCHIVE_IN_USE` | A request to delete a workspace archive container named one an accepted export is still WRITING or an accepted restore is still READING. The refusal is not about the file's permissions or the caller's authority: an unlink races the other operation's own `open`, so the same request either destroys nothing observable or fails a job the caller did not start, and which of the two happens is invisible from the outside. Distinct from `REVISION_CONFLICT`, which says the bytes under this name are not the bytes the client saw — here they are exactly those bytes and something else is mid-way through them. An in-flight DOWNLOAD is deliberately NOT this refusal: it holds an open descriptor, so the bytes it is serving survive the unlink intact. | yes — after the named job reaches a terminal state |
