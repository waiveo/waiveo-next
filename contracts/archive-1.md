...
- The exact cryptographic primitives (ARC-010, ARC-013), the signing key's custody (ARC-021 draft-note), and the kit-rotation behavior (ARC-114 draft-note) are draft-note proposals pending confirmation against the platform's key-hierarchy contract once it exists; corpus cases exercise the shapes and orderings these rules produce, not a specific cipher implementation.
