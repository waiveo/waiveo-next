...
**[ARC-124]** A deployment MUST be able to bound how many archive containers it retains, and a host MUST apply that bound without an operator asking. Retention is expressed as a **keep-last** count, a **keep-days** age, or both; where both are given a container is retained if EITHER would retain it, because the two express different fears — "I want the last five whatever happens" and "I want a fortnight of cover" — and intersecting them would silently defeat whichever the operator was relying on. An **absent policy retains everything**, which is the behaviour every deployment has today, so gaining this feature MUST NOT delete an archive nobody asked it to delete. A host MUST NOT delete the NEWEST container under any policy, including a `keep_last` of zero: a policy that can empty the archive directory is a policy that can destroy the only copy of a workspace, and no retention setting is worth that. The sweep MUST report what it removed, by name, through the same operator-visible channel an export reports itself — a backup that vanishes silently is indistinguishable from one that was never taken, which is exactly the doubt an archive exists to remove. A host MAY apply the sweep whenever it likes and MUST apply it after a successful export; a deployment that never exports again therefore never sweeps, which is stated here rather than left to be discovered, because it means retention bounds the archive set's GROWTH and is not a guarantee about its size on a box that has gone quiet. Retention MUST NOT reach anything but archive containers: it is a disk-reclamation policy over exports, never over the workspace's own rows.
