...
**[EVT-105a]** An SSE stream that has sent nothing for a bounded idle interval MUST send an SSE comment line (a line beginning `:`, which every conforming client ignores) as a keepalive. This is the SSE binding's counterpart to EVT-095's WS ping, and it is REQUIRED where the ping is optional because the two bindings are not symmetric: WS is bidirectional, so either peer can probe an idle connection and a subscriber can tell a live one from a dead one on its own. SSE has no client-to-server frame at all after the initial request (EVT-100), so absent a server-sent keepalive an idle stream, a stream the server is holding (EVT-142a), and a connection an intermediary dropped an hour ago are the same zero bytes on the wire — and a subscriber that cannot distinguish them cannot know it has stopped receiving events it is entitled to. A comment carries no event and no `id:`, so it neither advances a client's `Last-Event-ID` nor reaches its event handling.

*draft-note: EVT-105a's interval is not fixed by any normative source yet. Proposed: the same 30 seconds EVT-095 fixes for the WS ping, so one cadence describes an idle connection on either binding.*
...
**[EVT-142a]** EVT-134a requires a gap marker's `to_id` to name an `id` inside the subscriber's own visible set (EVT-120). For a mid-stream `buffer_exceeded` marker that is a rule about *timing* as well as about *value*: at the instant the loss is detected the retained log may hold nothing this subscriber may read, and then no `id` exists that its marker could name. A server MUST NOT resolve that by naming an `id` from outside the visible set — which would report that an event the subscriber may not read exists and, `id` being time-ordered (EVT-011), roughly when it was recorded — and MUST NOT resolve it by dropping the marker, which is EVT-143's silent loss. It MUST defer the marker, and emit it immediately ahead of the first subsequent retained event INSIDE that subscriber's visible set (EVT-120), with that event's `id` as `to_id` — the same set the connect-time resolution names a resume point from, so the two paths cannot reach different conclusions about what an id may name. Note this is the VISIBLE set and not the subscriber's own `selector`/`schemas` filter: a marker may therefore name an `id` the subscriber is not itself sent, which is deliberate — that `id` is one the subscriber MAY read and can resume from, and choosing the narrower filter would make the marker's `to_id` un-nameable whenever a subscriber's filter happens to exclude everything retained above the loss, reintroducing the very deferral this requirement bounds. `from_id` is fixed when the loss is detected, at the subscriber's own last-known point (EVT-140), and does not move while the marker is deferred.

Deferring a marker MUST NOT hold the subscriber's delivery position: the server continues to consider and deliver the retained tail normally while the marker is pending, so a deferral costs the subscriber nothing but the marker's timing. A server that instead froze the subscriber's position would re-derive the same unchanged answer from a growing tail on every wake, and would do so for a subscriber that is by construction receiving nothing.

A deferral MUST be bounded. A marker still without a resume point inside the visible set at that bound MUST end the connection rather than remain pending. On a binding that carries a close reason the connection MUST be closed with `SLOW_CONSUMER_DISCONNECTED` (Error taxonomy); on a binding that carries none — the SSE binding has no close frame — the server MUST end the stream and MUST record the disposition where an operator can find it, since the client sees only an ended stream and cannot otherwise distinguish this from a dropped connection. This is EVT-142's own disposition for the same underlying condition — a subscriber that fell behind live delivery and whose undelivered events were dropped — and it is the mid-stream counterpart of the resume-time rule that a `resume_from` older than the retention window with nothing recoverable above it is REFUSED (EVT-134) rather than gapped at an `id` that cannot be named. An unbounded deferral would be indistinguishable from silent loss: the subscriber stays connected to a stream that has already lost something and will never say so.

*draft-note: EVT-142a's deferral bound is not fixed by any normative source yet. Proposed: the same 30 seconds EVT-142's own draft-note proposes for its backed-up-writes bound — both bound one condition (a subscriber that has fallen behind live delivery), so one number rather than two.*
