...
A deferral MUST be bounded. A marker still without a resume point inside the visible set at that bound MUST end the connection with `SLOW_CONSUMER_DISCONNECTED` (Error taxonomy) rather than remain pending. This is EVT-142's own disposition for the same underlying condition — a subscriber that fell behind live delivery and whose undelivered events were dropped — and it is the mid-stream counterpart of the resume-time rule that a `resume_from` older than the retention window with nothing recoverable above it is REFUSED (EVT-134) rather than gapped at an `id` that cannot be named. An unbounded deferral would be indistinguishable from silent loss: the subscriber stays connected to a stream that has already lost something and will never say so.
