...
func driveREL056(rep *report.Report, cases map[string]corpus.Case) {
	...
	if err := store.ApplyGeneration(oldPair.Generation, oldPair.Hash, []byte(rel056OldScreenProgramsJSON), nil, nil); err != nil {
	...
	if err := store.ApplyGeneration(newPair.Generation, newPair.Hash, newProgramsJSON, nil, nil); err != nil {
		close(stop)
		wg.Wait()
