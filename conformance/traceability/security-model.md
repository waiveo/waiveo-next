...
records all eight as driven. Only THREE rows flip to `covered`, and the gap
between "eight cases pass" and "three rows covered" is the point of this note.

`covered` here keeps `README.md`'s meaning — a listed case exercises this
requirement today — with one addition this contract forces: the requirement
must be something the SHIPPED APP actually does. Most of security-model/1's
mechanisms exist in this tree only as components with no caller:
