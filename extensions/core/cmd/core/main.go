// Command core is the runtime of waiveo/core — the platform's own functions,
// shipped as ONE signed first-party extension rather than several small ones.
//
// # Why bundled
//
// The owner's call (2026-08-17): an operator should see a single core extension,
// not a scatter. So each platform function this extension owns contributes its
// own settings page, its own data collection and its own actions to one
// manifest, and this process routes every invocation to the function that owns
// it. Backups is the first; the shape is built to take the next without a second
// extension appearing in the operator's list.
//
// # What stays in core
//
// The extension owns POLICY — when a backup runs, how many are kept. The engine
// behind it stays in the platform: the archive writer, its encryption and the
// data key are never reachable by anything installed, which is the whole point
// of the division. This runtime therefore performs no cryptography and holds no
// key; it reads its own policy and calls the platform.
//
// # The lifecycle (identical to every code-carrying extension)
//
...
//  3. Long-poll GET /api/v1/extension-invocations/pending; answer each at
//     POST /api/v1/extension-invocations/{id}/result.
...
func main() {
	...
	if base == "" {
		fmt.Fprintln(os.Stderr, "core: WAIVEO_API_BASE_URL is not set; this process must be started by the pack host")
		os.Exit(2)
	}
	...
	if err != nil && code == "" {
		fmt.Fprintf(os.Stderr, "core: read grant code from stdin: %v\n", err)
		os.Exit(2)
	}
	code = trimEOL(code)
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "core: redeem tier grant: %v\n", err)
		os.Exit(1)
	}
...
func serve(ctx context.Context, client *http.Client, base string, sess session) error {
	...
	for {
		...
		if err := report(ctx, client, base, sess.Token, inv.InvocationID, result, errCode, errMsg); err != nil {
			...
			fmt.Fprintf(os.Stderr, "core: report %s: %v\n", inv.InvocationID, err)
		}
	}
}
...
func runBackup(ctx context.Context, client *http.Client, base string, sess session, inv invocation) (json.RawMessage, string, string) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet,
		base+"/api/v1/extensions/"+sess.PackID+"/data/backups", nil)
	if err != nil {
